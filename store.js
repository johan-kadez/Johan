...
if (SITE_CONFIG.backgroundUrl) {
  document.documentElement.style.setProperty("--site-bg-image", `url("${SITE_CONFIG.backgroundUrl.replace(/"/g, '\\"')}")`);
  document.body.classList.add("custom-bg");
}

const waSocial = $("waSocial");
if (waSocial) {
  const num = String(SITE_CONFIG.whatsappNumber || "").replace(/\D/g, "");
  if (num) waSocial.href = `https://wa.me/${num}`;
}
...
function ensureDetailPopup() {
  ...
  el.innerHTML = `
    <div class="product-detail-sheet" role="dialog" aria-modal="true" aria-label="Detail produk">
      ...
      <div class="detail-content">
        ...
        <p class="detail-spec" id="detailSpec"></p>
        <button type="button" class="gold-btn detail-order" id="detailOrder">Pesan via WhatsApp</button>
      </div>
    </div>
  `;
...
function getImages(p) {
  ...
  const all = [p.image, ...raw].map(x => String(x || "").trim()).filter(Boolean);
  return [...new Set(all)];
}
...
function openDetail(id) {
  ...
  if (!detailImages.length) detailImages = ["https://placehold.co/900x700?text=Johan+Service"];
  detailIndex = 0;

  $("detailCategory").textContent =
    p.normalizedCategory === "jasa" ? "JASA CPM" :
    p.normalizedCategory === "cpm1" ? "CPM 1" : "CPM 2";
  ...
  $("detailPrice").innerHTML =
    disc > 0 && normal > 0 && disc < normal
      ? `<span class="price-old">${rupiah(normal)}</span><span class="price-new">${rupiah(disc)}</span>`
      : normal ? `<span class="price-new">${rupiah(normal)}</span>` : "";

  $("detailStock").textContent =
    p.normalizedCategory === "jasa" ? "" : (Number(p.stock) > 0 ? "Tersedia" : "Stok habis");
  ...
  $("detailOrder").onclick = () => orderWhatsApp(p.id);
...
function renderGallery() {
  ...
  track.innerHTML = detailImages.map(src => `<div class="detail-slide"><img src="${escapeHtml(src)}" alt="Produk"></div>`).join("");
  track.style.transform = `translateX(-${detailIndex * 100}%)`;
  $("galleryCounter").textContent = `${detailIndex + 1} / ${detailImages.length}`;
  $("galleryPrev").classList.toggle("hidden", detailImages.length < 2);
  $("galleryNext").classList.toggle("hidden", detailImages.length < 2);
}

function moveGallery(step) {
  if (detailImages.length < 2) return;
  detailIndex = (detailIndex + step + detailImages.length) % detailImages.length;
  renderGallery();
}
...
async function loadProducts() {
  try {
    ...
    console.error(e);
    $("grid").innerHTML = "<p class='empty-state'>Gagal memuat produk. Periksa konfigurasi Firebase.</p>";
  }
}
...
function render() {
  ...
  const list = products.filter(p =>
    ...
    `${p.name || ""} ${p.specification || ""} ${p.description || ""}`.toLowerCase().includes(q)
  );

  $("grid").innerHTML = list.map(p => {
    const imgs = getImages(p);
    const image = escapeHtml(imgs[0] || "https://placehold.co/900x700?text=Johan+Service");
    ...
    const btn = `<button class="add" data-view="${escapeHtml(p.id)}" type="button">Lihat</button>`;
    ...
    if (p.normalizedCategory === "jasa") {
      return `
        <article class="product-card service-card">
          ...
          <div class="body">
            <div class="meta-row"><span class="tag">${catLabel}</span>${watermark}</div>
            <h3>${name || "Jasa CPM"}</h3>
            <p class="description">${escapeHtml(p.description || "")}</p>
            ${displayPrice ? `<p class="price">${displayPrice}</p>` : ""}
            ${btn}
          </div>
        </article>
      `;
    ...
    return `
      <article class="product-card card">
        ...
        <div class="body">
          <div class="meta-row"><span class="tag">${catLabel}</span>${watermark}</div>
          <h3>${name}</h3>
          <p class="spec">${escapeHtml(p.specification || "Specification belum tersedia.")}</p>
...
function orderWhatsApp(id) {
  ...
  const finalPrice =
    Number(p.discountPrice) > 0 && Number(p.price) > 0 && Number(p.discountPrice) < Number(p.price)
      ? p.discountPrice
      : p.price;
